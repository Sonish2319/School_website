...
// controllers/aboutus/aboutus.controller.js
const { AboutUs } = require('../../models/aboutus1/aboutus');

const createAboutUs = async (req, res) => {
  try {
    const { page_title, status, ...rest } = req.body;
...
const updateAboutUs = async (req, res) => {
  try {
    ...
    // combine images (old + new)
    let oldImages = [];
    if (about.images) {
      try { oldImages = JSON.parse(about.images); } catch(e){ oldImages = []; }
    }
    const newImages = req.files ? req.files.map(f => `/uploads/${f.filename}`) : [];
    const images = JSON.stringify([...oldImages, ...newImages]);
