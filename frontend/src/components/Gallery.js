...
//   const { data: schoolData, error: heroError, loading: heroLoading } = useFetchData('/api/home/gallery');
          ...
          {schoolData?.map((image, index) => (
            ...
            >
              <div className={`relative ${
                ...
                <Image
                  src={`${BASE_URL_MEDIA}${image.images}`}
                  alt={image.sub_text || 'Gallery Image'}
                  fill
                  className="object-cover hover:scale-105 transition-transform duration-300"
                />
                ...
                {/* Sub Text on Hover */}
                <div className="absolute bottom-0 left-0 right-0 px-4 py-2 text-white text-sm opacity-0 group-hover:opacity-100 transition-opacity">
                  {image.sub_text}
                </div>
              </div>
            </div>
